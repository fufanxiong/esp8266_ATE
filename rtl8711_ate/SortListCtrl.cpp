...
bool CSortListCtrl::DeleteColumn(int col)
{
	if(m_iNumColumns) m_iNumColumns -= 1;
	return(!!CListCtrl::DeleteColumn(col));
}
...
//	GetDrawColors(nItem, nSubItem, crText, crBkgnd);
	...
	if (!str.IsEmpty())
	{
		...
		if (nFmt == HDF_CENTER) nFormat |= DT_CENTER;
		else if (nFmt == HDF_LEFT) nFormat |= DT_LEFT;
		else nFormat |= DT_RIGHT;
...
void CSortListCtrl::SetItemColor(int nItem, int nSubItem, COLORREF colorText, COLORREF colorBkgnd)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	...
	if (pid)
	{
		(pid->crText)[nSubItem]	= (colorText == TT_DEFAULT) ? crWindowText : colorText;
		(pid->crBak)[nSubItem]	= (colorBkgnd == BK_DEFAULT) ? crWindow : colorBkgnd;
	}
