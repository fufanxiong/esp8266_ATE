...
// Does not correspond to an esquared_error code
#define TE2_ERROR_SPIUNLOCK 100
...
#define TE2_ADDR_WIDTH_UNSPECIFIED 0                    // Use auto-detection algorithm
static const uint32 TE2_PS_HEADER_NONE = 0xFFFFFFFF;    // Do not write the PS header
...
/*******************************************************************************
    ...
    Description :   One of the te2Open... set of functions which has to be
                    called to initialise the SPI interface. One of these
                    functions must be called before any other operations can
                    be performed. 
                    <p>To be used (only) to open a SPI connection to a
                    device which is not SPI-locked.
                    <p>te2OpenTrans can also be used to open a SPI
                    connection with a device which is not SPI-locked.
                    <p>te2OpenSpiUnlock or te2OpenSpiUnlockTrans should be
                    used to open a SPI connection with a SPI-locked device.
...
/*******************************************************************************
    ...
    Function :      int32 te2OpenSpiUnlock(int32 transport, int32 port,
                                           int32 device,
    ...
    Description :   One of the te2Open... set of functions which has to be
                    ...
                    <p>To be used (only) to open a SPI connection to a
                    SPI-locked chip.
                    <p> te2OpenSpiUnlockTrans can also be used to open a SPI
                    connection with a SPI-locked device.
                    <p>te2Open or te2OpenTrans should be used to open a SPI
                    connection with a device which is not SPI-locked.

//*****************************************************************************/
TESTE2_API(int32) te2OpenSpiUnlock(int32 transport, int32 port, int32 device,
                                   const char* unlockKey);
...
/*******************************************************************************
    ...
    Description :   One of the te2Open... set of functions which has to be
                    ...
                    <p>To be used (only) to open a SPI connection to a
                    device which is not SPI-locked.
                    <p>te2Open can also be used to open a SPI
                    connection with a device which is not SPI-locked.
                    <p>te2OpenSpiUnlock or te2OpenSpiUnlockTrans should be
                    used to open a SPI connection with a SPI-locked device.
...
/*******************************************************************************
    ...
    Function :      int32 te2OpenSpiUnlockTrans(const char *trans,
                                                const char* unlockKey)
...
//*****************************************************************************/
TESTE2_API(int32) te2OpenSpiUnlockTrans(const char *trans,
                                        const char* unlockKey);
...
/*******************************************************************************
    ...
    Description :   This function is used to write the special header - as
                    required by BlueCore firmware, to the EEPROM device. Function
                    te2WriteBcPsHeader replaces te2WriteBcHeader and can be used to
                    write both the EEPROM and PS headers.
...
/*******************************************************************************
    ...
    Function :      int32 te2WriteBcPsHeader(uint16 addrWidth, uint32 appSize)

    Parameters :    addrWidth -
                        The address width of the EEPROM.
                        <p>Valid values 8 through 19, or TE2_ADDR_WIDTH_UNSPECIFIED
                        (use auto-detection algorithm).

                    appSize -
                        The size reserved for the VM application.
                        <p>Valid values are 0 - 65535, or TE2_PS_HEADER_NONE (do
                        not write the PS header).

    Returns :       Returns 0 if the operation completed successfully,
                    otherwise one of the error codes defined in TestE2.h. Call 
                    te2GetLastError to get an error description.

    Description :   This function is used to write the special EEPROM header and PS
                    header as required by BlueCore firmware, to the EEPROM device.
                    <p>This function is equivalent to BCCMD_E2_DEVICE provided by
                    firmware enhanced to allow appSize other than zero to be
                    specified.
                    <p>Previously firmware would write the PS header at boot time
                    if it was found to have a EEPROM header but no valid PS header.
                    This behaviour was moved from boot time to be done explicitly
                    by BCCMD_E2_DEVICE.
                    This change is from firmware build id 8891 for CSR8311,
                    CSR8510, CSR881x; build id 9079 for CSR8345, CSR8350, CSR86xx;
                    build id 8668 for CSR5370.
                   
//*****************************************************************************/
TESTE2_API(int32) te2WriteBcPsHeader(uint16 addrWidth, uint32 appSize);
...
*******************************************************************************/
TESTE2_API(const char *) te2GetLastError(void);
